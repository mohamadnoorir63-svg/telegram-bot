...
async def sudo_bot_call(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """وقتی مدیر اصلی یا سودو بگه 'ربات' — پاسخ‌های ویژه فقط برای اونا"""
    ADMIN_ID = int(os.getenv("ADMIN_ID", "7089376754"))
    user_id = update.effective_user.id

    # ✅ فقط مدیر اصلی یا سودوها مجازند
    if user_id != ADMIN_ID and user_id not in SUDO_IDS:
        return  # فقط مدیر اصلی یا سودوها می‌تونن فعالش کنن
...
async def toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ فقط مدیر اصلی یا سودوها مجازند
    if user.id != ADMIN_ID and user.id not in SUDO_IDS:
    ...
    try:
        ...
        if isinstance(groups, list):
            ...
            for g in valid_groups:
                ...
                text += f"🏠 <b>گروه:</b> {title}\n👥 <b>اعضا:</b> {members}\n🕓 <b>آخرین فعالیت:</b> {last_active}\n\n"
        ...
        text += f"━━━━━━━━━━━━━━━\n📨 <i>درخواست توسط:</i> <b>{user.first_name}</b> (<code>{user_id}</code>)"
...
async def list_sudos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🎛 فعال‌سازی پنل راهنمای فارسی با گفتن "پنل"
    application.add_handler(
        MessageHandler(filters.TEXT & filters.Regex(r"^پنل$"), panel_menu),
        group=-3
    )

    # 🎛 کنترل دکمه‌های داخل پنل (prefix جدید Tastatur)
    application.add_handler(
        CallbackQueryHandler(panel_buttons, pattern="^tastatur_"),
        group=-3
    )
