...
🧾 این کیه (ریپلای)
🏷 لقب [متن] (ریپلای) | لقب
اصل [متن] (ریپلای)   -> ثبت اصل برای کاربر
اصل                   (ریپلای)   -> نمایش اصل کاربر
"""
...
def id_cmd(m):
    try:
        ...
        # اگر عکس پروفایل هست، ارسال کن با کپشنِ آیدی
...
def group_link(m):
    try:
        ...
        bot.reply_to(m,"❗ نتوانستم لینک بگیرم. (ربات باید ادمین با مجوز دعوت باشد)")
...
asls = {}     # chat_id -> { user_id: asl_text }   <-- اضافه: نگهداری "اصل"
...
def whoami(m):
    ...
    extra = ""
    my_asl = asls.get(m.chat.id,{}).get(m.from_user.id)
    if my_asl:
        extra = f"\n🏷 اصل: {my_asl}"
    bot.reply_to(m,f"شما {role} هستید.{extra}")
...
def whois(m):
    ...
    aslval = asls.get(m.chat.id,{}).get(uid)
    extra = ""
    if nick: extra += f"\n🏷 لقب: {nick}"
    if aslval: extra += f"\n🏷 اصل: {aslval}"
...
# ========= اصل (جدید) =========
@bot.message_handler(func=lambda m: m.reply_to_message and cmd_text(m).startswith("اصل "))
def set_asl(m):
    # فقط ادمین/سودو می‌تواند «اصل» کاربر را ثبت کند
    if not is_admin(m.chat.id,m.from_user.id): return
    uid = m.reply_to_message.from_user.id
    asl_text = cmd_text(m).replace("اصل ","",1).strip()
    if not asl_text:
        return bot.reply_to(m,"❗ متن اصل را وارد کن. مثال: `اصل عضو قدیمی`")
    asls.setdefault(m.chat.id, {})[uid] = asl_text
    bot.reply_to(m, f"✅ اصل برای <code>{uid}</code> ثبت شد: {asl_text}")

@bot.message_handler(func=lambda m: m.reply_to_message and cmd_text(m)=="اصل")
def get_asl(m):
    uid = m.reply_to_message.from_user.id
    asl_text = asls.get(m.chat.id, {}).get(uid)
    if asl_text:
        bot.reply_to(m, f"🏷 اصل: {asl_text}")
    else:
        bot.reply_to(m, "ℹ️ برای این کاربر «اصل» ثبت نشده.")
...
def clear_custom(m):
    ...
    try:
        ...
        return bot.reply_to(m,"❗ عدد معتبر وارد کن.")
    if num<=0:
        return bot.reply_to(m,"❗ عدد باید بیشتر از صفر باشد.")
    if num>9999:
        num=9999
    bulk_delete(m,num)
...
def add_sudo(m):
    ...
    try:
        ...
        return bot.reply_to(m,"❗ آیدی نامعتبر")
    sudo_ids.add(uid)
