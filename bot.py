...
def start_cmd(m):
    data = load_data()
    data["users"] = list(set(data.get("users", [])) | {m.from_user.id})
    save_data(data)
    bot.reply_to(m, "سلام 👋 ربات مدیریتی فعال است.")# ================== 🎉 خوشامد ==================
