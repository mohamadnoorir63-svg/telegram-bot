...
async def feature_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "feature_info":
        ...
        await query.message.reply_text(text, parse_mode="HTML")
      # ======================= 💫 خنگول فارسی Ultra Cloud+ v9.9 — ماژول مدیریتی =======================
from telegram import ChatPermissions, Update, InputFile
from telegram.ext import ContextTypes
import asyncio, os, json

# ===== تنظیمات =====
ADMIN_ID = 123456789  # ← آیدی عددی مدیر اصلی‌ات
CLEAN_DONE_PHOTO = "clean_done.jpg"
...
async def cmd_alias(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        return await update.message.reply_text("📘 مثال: /alias ban بن")

    base_cmd = context.args[0].strip().lstrip("/")
    alias_name = context.args[1].strip().lstrip("/")

    ALIASES[alias_name] = "/" + base_cmd
    save_json(ALIASES_FILE, ALIASES)
    await update.message.reply_text(f"✅ دستور مستعار «{alias_name}» برای /{base_cmd} ساخته شد!")

async def cmd_unalias(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context):
        return await update.message.reply_text("⛔ فقط مدیران می‌تونن alias حذف کنن!")
    if not context.args:
        return await update.message.reply_text("📘 مثال: /unalias بن")

    alias_name = context.args[0].strip().lstrip("/")
    if alias_name in ALIASES:
        del ALIASES[alias_name]
        save_json(ALIASES_FILE, ALIASES)
        await update.message.reply_text(f"🗑 دستور مستعار {alias_name} حذف شد.")
    else:
        await update.message.reply_text("⚠️ همچین مستعاری وجود نداره!")

async def cmd_list_alias(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context):
        return
    text = "🧠 <b>لیست aliasها:</b>\n\n"
    for a, c in ALIASES.items():
        text += f"▫️ <b>{a}</b> ➜ <code>{c}</code>\n"
...
async def alias_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd in ALIASES:
        new_text = ALIASES[cmd] + text[len(cmd):]
...
async def cmd_ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context):
        return await update.message.reply_text("⛔ فقط مدیران مجازند.")
    if not update.message.reply_to_message:
        return await update.message.reply_text("🔸 روی پیام کاربر ریپلای کن تا بن شه.")
...
async def cmd_unban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        return await update.message.reply_text("🔸 روی پیام کاربر ریپلای کن تا از بن خارج شه.")
    user = update.message.reply_to_message.from_user
    try:
...
async def cmd_mute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context): return
    if not update.message.reply_to_message:
        return await update.message.reply_text("🎧 ریپلای کن تا بی‌صدا بشه.")
    user = update.message.reply_to_message.from_user
...
async def cmd_unmute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context): return
    if not update.message.reply_to_message:
        return await update.message.reply_text("🎤 ریپلای کن تا آزاد شه.")
    user = update.message.reply_to_message.from_user
...
async def cmd_warn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context): return
    if not update.message.reply_to_message:
        return await update.message.reply_text("📍 ریپلای کن تا اخطار بگیره.")
    user = update.message.reply_to_message.from_user
    cid, uid = str(update.effective_chat.id), str(user.id)
    warnings.setdefault(cid, {})
    warnings[cid][uid] = warnings[cid].get(uid, 0) + 1
    save_json(WARN_FILE, warnings)
    count = warnings[cid][uid]
    msg = f"⚠️ {user.first_name} اخطار گرفت ({count}/3)"
    if count >= 3:
        await context.bot.ban_chat_member(update.effective_chat.id, user.id)
        msg += "\n🚫 به ۳ اخطار رسید و بن شد!"
    await update.message.reply_text(msg)

async def cmd_unwarn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context): return
    if not update.message.reply_to_message:
        return await update.message.reply_text("📍 ریپلای کن تا اخطار حذف شه.")
    user = update.message.reply_to_message.from_user
    cid, uid = str(update.effective_chat.id), str(user.id)
    if cid in warnings and uid in warnings[cid]:
        warnings[cid][uid] = max(0, warnings[cid][uid] - 1)
        save_json(WARN_FILE, warnings)
...
async def cmd_purgeall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("💣 در حال پاکسازی کل گروه... لطفاً صبر کنید ⏳")
    chat_id = update.effective_chat.id
    total = 0
    step = 5000
    ...
    while last > 1:
        ids = range(last, max(1, last - step), -1)
        results = await asyncio.gather(
            *[context.bot.delete_message(chat_id, i) for i in ids],
            return_exceptions=True
        )
        total += sum(1 for r in results if r is None)
        last -= step
        await msg.edit_text(f"🧹 در حال حذف... {total:,} پیام پاک شد 🌀")
        if last <= 1:
            break
    ...
    caption = (
        ...
        f"🧹 {total:,} پیام حذف شد.\n"
        f"👑 مدیر: <a href='tg://user?id={update.effective_user.id}'>{update.effective_user.first_name}</a>"
    )
...
if __name__ == "__main__":
    print("🤖 خنگول فارسی 8.9 Cloud+ Supreme Ultra Stable آماده‌ست ...")

    app = ApplicationBuilder().token(TOKEN).build()
    app.add_error_handler(handle_error)

    # =================== 🧠 دستورات مدیریتی و کنترلی ===================
    ...
    # =================== 🔒 سیستم محافظ گروه (Ban / Mute / Warn / Purge) ===================
    ...
    app.add_handler(CommandHandler("purgeall", cmd_purgeall))

    # ⚙️ سیستم alias (ساخت و اجرای دستورات بدون /)
    app.add_handler(CommandHandler("alias", cmd_alias))
    app.add_handler(CommandHandler("unalias", cmd_unalias))
    app.add_handler(CommandHandler("listalias", cmd_list_alias))
    # این باید قبل از reply بیاد تا دستورهای فارسی بدون / تشخیص داده بشن
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, alias_router), group=-1)

    # =================== 🪄 پنل قابلیت‌ها و ماژول‌ها ===================
    ...
    # =================== 🎉 سیستم خوشامد و قوانین ===================
    # قبل از هندلر عمومی ثبت می‌شود تا همیشه اولویت بالاتر داشته باشد
    ...
    # =================== 🧠 هندلر اصلی پاسخ‌دهی (در انتها قرار گیرد) ===================
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, reply), group=10)

    # =================== 🌙 اجرای وظایف زمان استارت ===================
