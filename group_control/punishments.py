...
async def handle_punishments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target_user.id in SUDO_IDS:
        return await msg.reply_text("🚫 این کاربر در لیست سودو است.")
    try:
        tm = await context.bot.get_chat_member(chat.id, target_user.id)
        if tm.status in ("creator", "administrator"):
            return await msg.reply_text("🛡 امکان اجرای دستور روی ادمین وجود ندارد.")
    except Exception:
        pass

    # 📡 مقدار ارسال برای یوزربات (username یا ID)
