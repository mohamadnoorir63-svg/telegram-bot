...
async def group_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ تشخیص قفل‌ها به‌صورت مستقیم (مثل: قفل لینک / بازکردن عکس)
    if text.startswith("قفل") or text.startswith("باز"):
        return await handle_locks_with_alias(update, context)

    # ✅ بررسی aliasها برای سایر دستورات
    for cmd, aliases in ALIASES.items():
        for alias in aliases:
            if text.startswith(alias):
                # استخراج آرگومان‌ها بعد از دستور
                context.args = text.replace(alias, "", 1).strip().split()
                return await execute_command(cmd, update, context)

    # ✅ اگر هیچ دستور شناخته‌شده‌ای نبود، هیچی انجام نده
    return
