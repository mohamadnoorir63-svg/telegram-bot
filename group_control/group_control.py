...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin:
        return  # مدیرها از قفل‌ها مستثنا هستند
    ...
    # 🚫 عکس / ویدیو / فایل / ویس / گیف / استیکر / فوروارد
    ...
    # 🚫 قفل تبلیغ
    if locks.get("ads") and any(x in text for x in ["joinchat", "promo", "channel", "bot?start="]):
        return await _del_msg(update, "🚫 تبلیغ ممنوع است.")

    # 🚫 قفل یوزرنیم و منشن
    if (locks.get("usernames") or locks.get("mention")) and "@" in text:
        return await _del_msg(update, "🚫 منشن یا یوزرنیم ممنوع است.")
    ...
    if locks.get("reply") and msg.reply_to_message:
        return await _del_msg(update, "🚫 پاسخ دادن (ریپلای) ممنوع است.")
...
# ───────────────── بررسی و اعمال قفل‌ها (Helper) ─────────────────
async def _apply_locks_if_needed(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, chat, user) -> bool:
    """
    اگر قفلی نقض شده باشد پیام را حذف می‌کند و True برمی‌گرداند.
    اگر کاری نبود انجام دهد، False برمی‌گرداند.
    """
    locks = _get_locks(chat.id)
    if not any(locks.values()):
        return False
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """دستگاه مرکزی کنترل گروه — نسخه پایدار و نهایی با اعمال قفل‌ها در ابتدای مسیر"""
    if not update.message:
        return
    ...
    # ───────────────────── اعمال قفل‌ها در همان ابتدای مسیر ─────────────────────
    # اگر قفلی نقض شده باشد، پیام حذف و True برمی‌گرداند؛ پس همین‌جا خروج می‌کنیم.
    if await _apply_locks_if_needed(update, context, text, chat, user):
        return

    # ─────────────────────────────── دستورات مدیریتی پایه ───────────────────────────────
    ...
    # اگه پنل داری و کال‌بک‌هندلرش رو ثبت کرده‌ای:
    ...
    # در انتها: فقط بررسی alias و در صورت نگاشت، هدایت به همین تابع
