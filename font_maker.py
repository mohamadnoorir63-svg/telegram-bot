...
def generate_fonts(name):
    ...
    fonts = []
    for _ in range(50):
        # انتخاب یک نماد از هر گروه قبل اسم
        pre = "".join(random.choice(group) for group in pre_symbols_groups)
        # انتخاب یک نماد از هر گروه بعد اسم
        post = "".join(random.choice(group) for group in post_symbols_groups)

        # انتخاب فونت یونیکد برای کل اسم
        style = random.choice(unicode_styles)
        trans = str.maketrans(style[1], style[0])
        uname = name.translate(trans)

        # ترکیب نهایی: قبل + اسم + بعد
    ...
    for _ in range(50):  # تولید ۵۰ فونت شلوغ
        # انتخاب ۲-۳ نماد قبل و بعد اسم
        pre = "".join(random.choices(pre_symbols, k=random.randint(2,3)))
        post = "".join(random.choices(post_symbols, k=random.randint(2,3)))
        ...
        # ترکیب نهایی: قبل + اسم یونیکد + بعد
        final_font = f"{pre}{uname}{post}"
        fonts.append(final_font)
