...
def _download_audio_bytes(url):
    ...
    opts = {
        ...
        "postprocessor_args": ["-f", "mp3"],
        "prefer_ffmpeg": True,
        "forcefilename": True,
    }
    with yt_dlp.YoutubeDL(opts) as ydl:
        info = ydl.extract_info(url)
        # فایل را روی دیسک دانلود نکن، بلکه BytesIO
...
async def youtube_quality_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "yt_audio":
        ...
        await context.bot.send_audio(chat_id, audio=audio_bytes, caption=f"🎵 {info.get('title','Audio')}", reply_markup=get_add_btn(update.effective_chat.type))
        await status_msg.delete()
        return
    if choice == "yt_video":
        info, video_bytes = await loop.run_in_executor(executor, _download_video_bytes, url)
        await context.bot.send_video(chat_id, video=video_bytes, caption=f"🎬 {info.get('title','Video')}", reply_markup=get_add_btn(update.effective_chat.type))
        await status_msg.delete()
        return
