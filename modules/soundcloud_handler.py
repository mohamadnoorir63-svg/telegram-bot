...
# ================================
executor = ThreadPoolExecutor(max_workers=16)  # افزایش کارایی
...
BASE_OPTS = {
    ...
    "noplaylist": True,
    "overwrites": True,
    "concurrent_fragment_downloads": 24,  # افزایش سرعت
}
...
def _youtube_fallback_sync(query: str):
    opts = BASE_OPTS.copy()
    opts["concurrent_fragment_downloads"] = 24
...
async def soundcloud_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # SoundCloud نتیجه نداد → fallback YouTube
    if not result or not result.get("entries"):
        await msg.edit_text("⌛ SoundCloud نتیجه نداد، در حال جستجو در YouTube…")
        try:
